...
- *Project rule:* Joi validation (the `unknown`-until-validated rule is in the TS/Joi section above; the shared wrapper itself lives in `expertise-api-rest`) is a required security boundary for untrusted input, not just a type-safety nicety — never skip it because a value "looks fine" coming from an internal caller. It is **one layer, not the whole defense**: it doesn't replace authorization checks (domain-scoping in `proxy.ts`), DB-level constraints (a `@unique` still matters even though Joi checked shape — see `expertise-postgres-prisma`), or rate limiting on public endpoints (`expertise-api-rest`). A value can be perfectly valid Joi-wise and still be a request from someone who isn't allowed to make it.
